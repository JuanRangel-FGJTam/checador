...
<template>
	<div class="flex flex-col bg-white px-2 mb-4 rounded-md dark:bg-slate-6 dark:text-gray-200 dark:bg-gray-700 dark:border-gray-500" :class="[ shadow?'shadow':'', !whitOutBorder?'border':'' ]">
		...
		<div class="flex items-center select-none p-2 border-b text-gray-600 font-medium sticky top-0 z-10 bg-white dark:bg-gray-700 dark:text-gray-200 dark:border-gray-500" :class="[hiddenContent && isClosable ? 'border-none':'border-b', !isClosable?'justify-between':'']" v-on:click="headerClick" >
			<div v-if="isClosable">
