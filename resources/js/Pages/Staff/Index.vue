...
onMounted(()=>{
    form.page = props.filters.page ?? 0;
...
function reloadData(){
    ...
    debounce(()=>{
        ...
        if(form.search ){
            params.push(`s=${form.search}`);
        }
        
        // * reload the view
        router.visit("?" + params.join("&"), {
            method: 'get',
            only: ['employees', 'showPaginator', 'paginator'],
            preserveState: true,
            onError:(err)=>{
    ...
    <AuthenticatedLayout>
        ...
        <div class="px-4 py-4 rounded-lg min-h-screen max-w-screen-2xl mx-auto">
            ...
            <div class="grid grid-cols-3 gap-2 px-2 pt-2 pb-4 bg-white border-x border-t dark:bg-gray-700 dark:border-gray-500">
                <SearchInput class="col-span-3" placeHolder="Nombre, numero de empleado" v-on:search="handleInputSearch" :initialValue="form.search"/>
            </div>
